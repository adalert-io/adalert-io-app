...
const FIELD_MAP = [
  // Email/SMS
  { key: 'Send Email Alerts', label: <>Send me in <span className="font-bold">Email</span> alerts</>, id: 'email-alerts' },
  { key: 'Send SMS Alerts', label: <>Send me in <span className="font-bold">SMS</span> alerts <span className="font-normal text-xs">(critical alerts only)</span></>, id: 'sms-alerts' },
  // Severity
  { key: 'Severity Critical', label: 'Critical', id: 'critical', group: 'Severity' },
  ...
  // Type
  { key: 'Type Ad Performance', label: 'Ad Performance', id: 'ad-performance', group: 'Type' },
  { key: 'Type Brand Checker', label: 'Brand Checker', id: 'brand-checker', group: 'Type' },
  { key: 'Type Budget', label: 'Budget', id: 'budget', group: 'Type' },
  { key: 'Type KPI Trends', label: 'KPI Trends', id: 'kpi-trends', group: 'Type' },
  { key: 'Type Keyword Performance', label: 'Keyword Performance', id: 'keyword-performance', group: 'Type' },
  { key: 'Type Landing Page', label: 'Landing Page', id: 'landing-page', group: 'Type' },
  { key: 'Type Optimization Score', label: 'Optimization Score', id: 'optimization-score', group: 'Type' },
  { key: 'Type Policy', label: 'Policy', id: 'policy', group: 'Type' },
  { key: 'Type Serving Ads', label: 'Serving Ads', id: 'serving-ads', group: 'Type' },
];

export default function AlertsSubtab() {
  const { user } = useAuthStore();
  const { alertSettings, loading, error, fetchAlertSettings, updateAlertSettings, loadedUserId } = useAlertSettingsStore();
  ...
  useEffect(() => {
    if (alertSettings) {
      setLocalSettings(Object.fromEntries(
        FIELD_MAP.map(f => [f.key, !!alertSettings[f.key as keyof typeof alertSettings]])
      ));
    }
  }, [alertSettings]);
  ...
  const getFieldsByGroup = (group: string) => FIELD_MAP.filter(f => f.group === group);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-md p-8">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-8">
        <Card className="flex-1 p-4 border-2">
          <div className="flex flex-row items-center justify-center gap-3 w-full h-full">
            <Checkbox checked={!!localSettings['Send Email Alerts']} onCheckedChange={() => handleCheckbox('Send Email Alerts')} className={`mr-2 ${checkboxClass}`} id="email-alerts" />
        ...
        <Card className="flex-1 p-4 flex flex-col gap-2 border-2">
          <div className="flex items-center gap-3">
            <Checkbox checked={!!localSettings['Send SMS Alerts']} onCheckedChange={() => handleCheckbox('Send SMS Alerts')} className={`mr-3 ${checkboxClass}`} id="sms-alerts" />
          ...
          {getFieldsByGroup('Severity').map(f => (
            <Card key={f.key} className="flex items-center gap-3 p-4">
              <div className="flex flex-row items-center gap-3 w-full h-full">
                <Checkbox checked={!!localSettings[f.key]} onCheckedChange={() => handleCheckbox(f.key)} id={f.id} className={`mr-2 ${checkboxClass}`} />
                <label htmlFor={f.id} className="text-base font-medium select-none">{f.label}</label>
              </div>
            </Card>
          ))}
        </div>
      ...
      <div className="flex justify-start mt-8">
        <Button onClick={handleSave} disabled={saving || loading} className="bg-blue-600 text-white text-lg font-bold px-12 py-3 rounded shadow-md hover:bg-blue-700 transition-all">
          {saving ? 'Saving...' : 'Save'}
        </Button>
      </div>
    </div>
