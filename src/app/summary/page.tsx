...
export default function Summary() {
  ...
  function getDotColor(key: string | null) {
    if (["AccountIsOverPacing33PercentToDate", "AccountIsUnderPacing33PercentToDate"].includes(key || ""))
      return "#ede41b"
    if (["AccountIsOverPacing50PercentToDate", "AccountIsUnderPacing50PercentToDate"].includes(key || ""))
      return "#ff7f26"
    if (["AccountIsOverPacing75PercentToDate", "AccountIsUnderPacing75PercentToDate"].includes(key || ""))
      return "#eb0009"
    return "#1BC47D"
  }
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
          <div className="flex flex-col items-start w-full lg:w-auto">
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Ad Accounts</h1>
            ...
            <table className="min-w-full text-[0.75rem]">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-4 text-left font-semibold text-gray-700">Showing Ads</th>
                  <th className="px-4 py-4 text-left font-semibold text-gray-700">Impact</th>
                  <th className="px-4 py-4 text-left font-semibold text-gray-700">Budget Pacing</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {summaryLoading && accounts.length === 0 ? (
                  ...
                  pagedAccounts.map((acc) => (
                    <tr
                      key={acc.id}
                      className={`hover:bg-gray-50 transition-colors ${acc.isConnected ? "cursor-pointer" : "cursor-not-allowed opacity-75"}`}
